...
export default function CandidateDetail() {
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      {activeTab === "memory" && (
        <div className="space-y-5">
          {/* 1. Candidate Ground Truth Facts */}
          <div className="rounded-[8px] border border-[#E5E7EB] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.06)] dark:border-[#27272A] dark:bg-[#121215]">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-[#27272A]">
              <div className="flex items-center gap-2">
              ...
              {memoryGraph && memoryGraph.total_rounds_completed > 0 && (
                <span className="text-xs text-muted-foreground">
                  {memoryGraph.total_rounds_completed} {memoryGraph.total_rounds_completed === 1 ? "round" : "rounds"} completed
                </span>
              )}
            </div>
            ...
            {loadingMemory ? (
              <div className="py-8 text-center text-xs text-muted-foreground flex items-center justify-center gap-2">
                <Spinner className="h-4 w-4" /> Loading agent memory...
              </div>
            ) : !memoryGraph || memoryGraph.total_rounds_completed === 0 ? (
              <div className="py-8 text-center text-xs text-muted-foreground space-y-1">
                <p className="font-medium text-slate-700 dark:text-slate-300">No agent memory recorded yet</p>
                <p className="text-[11px] text-muted-foreground">
                  When this candidate completes an interview round, verified details will appear here.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 pt-4 text-xs">
                <div>
                  <div className="text-[11px] font-medium text-muted-foreground">Notice Period</div>
                  <div className="mt-1 font-semibold text-slate-900 dark:text-slate-100">
                    {memoryGraph.verified_facts.notice_period_days != null
                      ? `${memoryGraph.verified_facts.notice_period_days} days`
                      : "—"}
                ...
                <div>
                  <div className="text-[11px] font-medium text-muted-foreground">Expected Compensation</div>
                  <div className="mt-1 font-semibold text-slate-900 dark:text-slate-100">
                    {memoryGraph.verified_facts.expected_ctc_min || memoryGraph.verified_facts.expected_ctc_max
                      ? `₹${memoryGraph.verified_facts.expected_ctc_min || memoryGraph.verified_facts.expected_ctc_max} LPA`
                      : "—"}
                ...
                <div>
                  <div className="text-[11px] font-medium text-muted-foreground">Relocation &amp; Location</div>
                  <div className="mt-1 font-semibold text-slate-900 dark:text-slate-100">
                    {memoryGraph.verified_facts.open_to_relocation === true
                      ? `Open to relocate (${memoryGraph.verified_facts.location_confirmed || candidate?.location || "India"})`
                      : memoryGraph.verified_facts.location_confirmed || candidate?.location || "—"}
                  </div>
                </div>

                <div>
                  <div className="text-[11px] font-medium text-muted-foreground">Reason for Switching</div>
                  <div className="mt-1 font-semibold text-slate-900 dark:text-slate-100 truncate" title={memoryGraph.verified_facts.reason_for_switching || "—"}>
                    {memoryGraph.verified_facts.reason_for_switching || "Seeking career growth"}
                  </div>
                </div>
              </div>
          ...
          {/* 2. Verified Skills */}
          {memoryGraph && memoryGraph.skills_matrix && memoryGraph.skills_matrix.length > 0 && (
            <div className="rounded-[8px] border border-[#E5E7EB] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.06)] dark:border-[#27272A] dark:bg-[#121215]">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-[#27272A]">
                <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                  Verified Skills ({memoryGraph.skills_matrix.length})
                </h2>
              </div>
              ...
              <div className="pt-4 flex flex-wrap gap-2">
                {memoryGraph.skills_matrix.map((sk, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs text-slate-800 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200"
                  >
                    <span className="font-medium">{sk.skill}</span>
                    {sk.years && <span className="text-muted-foreground text-[11px]">({sk.years}y)</span>}
                  </span>
                ))}
              </div>
            </div>
          ...
          {/* 3. Live Voice Agent Briefing */}
          {memoryGraph && memoryGraph.briefing_text && (
            <div className="rounded-[8px] border border-[#E5E7EB] bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.06)] dark:border-[#27272A] dark:bg-[#121215]">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-[#27272A]">
                <div className="flex items-center gap-2">
                  <Bot className="h-4 w-4 text-slate-700 dark:text-slate-300" />
                  <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                    Voice Agent Briefing Prompt
                  </h2>
                </div>
              ...
              <div className="mt-4 rounded-md bg-slate-950 p-4 font-mono text-xs text-slate-200 leading-relaxed max-h-72 overflow-y-auto whitespace-pre-wrap dark:bg-black border border-slate-800">
